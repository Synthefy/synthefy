...
the size-explicit model slug in the request body (e.g. `synthefy/nori-30m`). The gateway is the only remote
...
- Hangs / read timeout → the hosted `synthefy/nori-30m` deployment is asleep or unhealthy; confirm a `curl` to the gateway returns 200.
